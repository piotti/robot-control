...
class RobotQueue:
    ...
    def __init__(self, master, cnsl_print, stacks):
        ...
        Button(button_frame, text="Open Queue File", command=self.on_open_queue).grid(row=0, column=0, sticky=E+W+N)
        self.queue_label = Label(button_frame, text='helo', font=("Consolas", 9))
        self.queue_label.grid(row=1, column=0, columnspan=3, sticky=E+W)
        self.start_btn = Button(button_frame, text='Start Process', command=self.on_process_start, state=DISABLED)
        self.start_btn.grid(row=0, column=1, sticky=E+W)
    ...
    def on_auto(self):
        ...
        # if self.process_when_resumed:
        #     self.process_when_resumed = False
            
            # print 'resuming process'
        if not self.executing and not self.stopped:
            print 'resuming'
            self.handle_api_call(self.process)
    ...
    def on_redo(self):
        ...
        # redo
        self.handle_api_call(self.process, redo=True)
    ...
    def handle_api_call(self, func, *args, **kwargs):
        ...
        try:
            ...
            self.process_api_error()
        except ValueError:
            self.show_err('Error - step %d: Couldn\'t parse value.' % self.step)
            self.process_api_error()
        except Exception:
            self.show_err('Error - step %d: An unknown error occured.' % self.step)
            traceback.print_exc()
            self.process_api_error()
    ...
    def process(self, redo=False):
        ...
        if self.stopped:
            return
        # if self.paused and not self.stepped:
        #     self.process_when_resumed = True
        #     return
        
        if not redo:
            line = self.qfm.get_next_line(not self.paused)
        else:
            line = self.qfm.repeat_line(not self.paused)
        if line is None:
            return
        ...
        # parse action
        status = self.api.parse_action(not self.paused, step, action, arg1, arg2, arg3)
        print 'status', status
        if status == 1:
        ...
        if not self.paused:
            ...
            # we're done executing until switched to auto or command is stepped, repeated
            self.executing = False
...
class Counter:
    ...
    def update(self):
        ...
        if self.hrs:
            ...
            self.timer_label['text'] = '%d seconds' % seconds
